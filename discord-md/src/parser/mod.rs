...
use nom::{
    ...
    combinator::{map, map_parser, not, opt, peek, rest, verify},
    multi::many0,
...
fn italics_star(i: &str) -> IResult<&str, ItalicsStar> {
    map(
        map_parser(
            delimited(
                tag("*"),
                verify(take_before1(tag("*")), |s: &str| !s.contains('\n')),
                tag("*"),
            ),
...
/// Parses a single underscore that is not followed by an alphanumeric character.
fn standalone_underscore(i: &str) -> IResult<&str, &str> {
    terminated(tag("_"), peek(not(alt((alphanumeric1, tag("_"))))))(i)
}
...
fn italics_underscore(i: &str) -> IResult<&str, ItalicsUnderscore> {
    map(
        map_parser(
            delimited(
                tag("_"),
                take_before1(standalone_underscore),
